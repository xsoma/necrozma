...
namespace hs {
	...
	_(client_state2, "client_state"); // client_state
	_(client_state_p2, "8B 3D ? ? ? ? 8A F9"); // A1 ? ? ? ? 8B 80 ? ? ? ? C3
	...
	_(content_type1, "Content-Type");
	_(group_vk, "https://vk.com/fatality_legend");
	_(open, "open");
	_(necrozma_su, "gamesense");
	_(necrozma_logo, "gamesense");
	...
	_(m_nFallbackPaintKit, "m_nFallbackPaintKit"); // DT_WeaponCSBaseGun
	_(CBaseWeaponWorldModel, "CBaseWeaponWorldModel"); // DT_WeaponCSBaseGun
	_(CBaseAttributableItem, "CBaseAttributableItem"); // DT_WeaponCSBaseGun
	_(CBaseCombatWeapon, "CBaseCombatWeapon"); // DT_WeaponCSBaseGun
	_(m_nModelIndex, "m_nModelIndex"); // DT_WeaponCSBaseGun
	_(m_iViewModelIndex, "m_iViewModelIndex"); // DT_WeaponCSBaseGun
	...
	// hwid
	_(necrozma_su_subs_check, "https://necrozma.su/subs_check"); // https://necrozma.su/subs_check
	...
	_(clear_deatch, "55 8B EC 83 EC 0C 53 56 8B 71 58"); // 85 C0 75 30 38 86
	_(pthis, "B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08"); // 85 C0 75 30 38 86
	_(find_hud_element, "55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39 77 28"); // 85 C0 75 30 38 86
}
...
namespace fnv_1a {
	...
	struct fnv1a< uint32_t > : public fnv_internal< uint32_t > {
		 static uint32_t hash(char const* string, const uint32_t val = default_offset_basis) {
			//auto test = hash(&string[1], (val ^ uint32_t(string[0])) * prime);
			//test = test;
			//return hash(&string[1], (val ^ uint32_t(string[0])) * prime);
			unsigned int t = 0;
