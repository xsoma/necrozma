...
Vector Ragebot::FullScan(animation* anims, int& hitbox, float& simtime, float& best_damage, float min_dmg) {
	...
	for (auto HitboxID : baim_hitboxes) {
		...
		for (int k = 0; k < Points.size(); k++)
		{
			...
			if ((info.m_damage > min_dmg && info.m_damage > body_damage))
			{
				hitbox = HitboxID;
				best_point = Points[k];
				body_damage = info.m_damage;
			}
		}
	}

	for (auto HitboxID : upper_hitboxes)
	{
		std::vector<Vector> Points = GetMultipoints(anims->player, HitboxID, BoneMatrix);
		for (int k = 0; k < Points.size(); k++)
		{
			auto info = g_AutoWall.Think(Points[k], anims->player);
			if ((info.m_damage > min_dmg && info.m_damage > head_damage))
			{
				hitbox = HitboxID;
				head_damage = info.m_damage;
			}
		}
	}

	if (CurrentSettings().adaptive_baim) {
		for (auto HitboxID : baim_hitboxes) {
			std::vector<Vector> Points = GetMultipoints(anims->player, HitboxID, BoneMatrix);
			for (int k = 0; k < Points.size(); k++)
			{
		...
		if (best_damage > 0) 
		{
			...
			else if (csgo->local->GetWeapon()->GetItemDefinitionIndex() == WEAPON_SSG08 && body_damage > health)
			{
				target_lethal = true;
				RestorePlayer(anims);
				return best_point;
			}
			else if (csgo->local->GetWeapon()->GetItemDefinitionIndex() == WEAPON_AWP && body_damage > 100)
			{
				target_lethal = true;
				RestorePlayer(anims);
				return best_point;
...
Vector Ragebot::GetAimVector(IBasePlayer* pTarget, float& simtime, Vector& origin, animation*& best_anims, int& hitbox)
{
	...
	{
		m_damage = csgo->weapon->IsZeus() ? 120.f : CurrentSettings().mindamage_override;
	}
	else
		m_damage = csgo->weapon->IsZeus() ? 120.f : CurrentSettings().mindamage;
...
string ShotSnapshot::get_info(bool miss) {
	string ret;
	if (miss)
		ret += hitbox_where_shot;

	if (vars.ragebot.posadj) {
		ret += " bt=" + std::to_string(backtrack);
	}
	return ret;
}
...
void Ragebot::Run()
{
	...
	if (hitbox != -1 && target_index != -1 && best_anims && current_aim_position != Vector(0, 0, 0))
	{
		...
		if (htchance && is_able_to_shoot)
		{
			...
			//interfaces.engine->SetViewAngles(Math::CalculateAngle(csgo->local->GetEyePosition(), current_aim_position) - csgo->local->GetPunchAngle() * 2.f);
