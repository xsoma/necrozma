...
bool c_slider::hovered_custom(int x, int y, int x2, int y2)
{
	if (should_render)
		if (!should_render())
			return false;
	...
	if (label.size() > 0)
		pos.y += after_text_offset;
	auto this_size = Vector2D(20, 15);
	return g_mouse.x > pos.x + x && g_mouse.y > pos.y + y && g_mouse.x < pos.x + x2 && g_mouse.y < pos.y + y2;
}
bool c_slider::update()
{
	...
	bool h = hovered();
	bool hcml = hovered_custom(-15, 0, 0, size.y / 2 - 2);
	bool hcpl = hovered_custom(size.x + 15, 0, size.x + 20, size.y / 2 - 2);
...
void c_slider::render() {
	...
	auto clr = color_t(vars.misc.accentcolor.get_red(), vars.misc.accentcolor.get_green(), vars.misc.accentcolor.get_blue(), alpha);
	auto clr2 = color_t(47, 47, 47, clamp(alpha - 100.f, 0.f, 255.f));
	if (label.size() > 0) {
	...
	g_Render->FilledRect(pos.x, pos.y, location, size.y - 2, clr);
	g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, size.y, color_t(47, 47, 47, alpha));
	...
	g_Render->DrawString(pos.x + size.x + 10, pos.y + size.y / 2 - 2, color_t(255, 255, 255, alpha), render::centered_x | render::centered_y | render::outline, fonts::menu_main, "+");
	g_Render->DrawString(pos.x - 10, pos.y + size.y / 2 - 2, color_t(255, 255, 255, alpha), render::centered_x | render::centered_y | render::outline, fonts::menu_main, "-");
