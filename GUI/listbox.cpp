...
void c_listbox::render() {
	...
	g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, size.y + 2, color_t(47, 47, 47, alpha));
	...
	g_Render->FilledRect(pos.x, pos.y, size.x, size.y,
		color_t(32, 32, 32, alpha));
	if (should_draw_scroll)
		pos.y -= scroll;
	for (int i = 0; i < elements.size(); i++) {
		g_Render->FilledRect(pos.x, pos.y, size.x, 20,
			color_t(40 + 10.f * animations[i], 40 + 10.f * animations[i], 40 + 10.f * animations[i], alpha));

		g_Render->DrawString(pos.x + 10, pos.y + 10,
			*(unsigned int*)value == i ? color_t(vars.misc.accentcolor.get_red(), vars.misc.accentcolor.get_green(), vars.misc.accentcolor.get_blue(), alpha) :
