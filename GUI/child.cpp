...
void c_child::render() {
	...
	auto clr = color_t(47, 47, 47, alpha);
	
	g_Render->FilledRect(pos.x, pos.y, size.x, size.y, color_t(18, 18, 18, (int)(wnd->get_transparency() * 2.55f)));
	...
	if (should_draw_scroll) {
		...
		g_Render->FilledRect(pos.x + size.x - 9, pos.y + 1, 8, size.y - 2, color_t(12, 8, 10, alpha));
		if (scroll.value > 0) {
