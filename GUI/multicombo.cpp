...
void c_multicombo::render() {
	...
	if (open) {
		...
		g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, size.y * (elements.size() + 1), color_t(47, 47, 47, new_alpha));
		g_Render->FilledRect(pos.x, pos.y, size.x, size.y, color_t(32.f, 32.f, 32.f, alpha));
		string format;
		int t = 0;
		...
		for (size_t i = 0; i < elements.size(); i++) {
			pos.y += size.y;
			g_Render->FilledRect(pos.x, pos.y, size.x, size.y, color_t(32.f + 15.f * animations[i], 32.f + 15.f * animations[i], 32.f + 15.f * animations[i], new_alpha));
			...
			g_Render->DrawString(pos.x + 10, pos.y + size.y / 2,
				((*(unsigned int*)value) & 1 << i) ?
				color_t(vars.misc.accentcolor.get_red(), vars.misc.accentcolor.get_green(), vars.misc.accentcolor.get_blue(), new_alpha) : clr2, render::centered_y,
