...
void c_combo::render() {
	...
	if (open) {
		...
		g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, (size.y * (elements.size() + 1)) + 2, color_t(47, 47, 47, new_alpha));
		g_Render->FilledRect(pos.x, pos.y, size.x, size.y, color_t(32, 32, 32, alpha));

		auto base = ImVec2(pos.x + size.x - 10, pos.y + size.y / 2);
		g_Render->_drawList->AddTriangleFilled(ImVec2(base.x - 2, base.y + 1), ImVec2(base.x + 2, base.y + 1), ImVec2(base.x, base.y - 2), color_t(255, 255, 255, new_alpha).u32());
		...
		for (size_t i = 0; i < elements.size(); i++) {
			...
			g_Render->FilledRect(pos.x, pos.y, size.x, size.y,
				color_t(32.f + 15.f * animations[i], 32.f + 15.f * animations[i], 32.f + 15.f * animations[i], new_alpha));

			auto clr2 = color_t(200 + animations[i] * 55.f, 200 + animations[i] * 55.f, 200 + animations[i] * 55.f, new_alpha);
			g_Render->DrawString(pos.x + 10, pos.y + size.y / 2, *(int*)value == i? color_t(vars.misc.accentcolor.get_red(), vars.misc.accentcolor.get_green(), vars.misc.accentcolor.get_blue(), new_alpha) : clr2, render::centered_y,fonts::menu_desc, elements[i].c_str());
		}
	}
	else {
		...
		g_Render->filled_rect_gradient(pos.x, pos.y, size.x, size.y, clr2, clr2, clr, clr);
		g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, size.y + 2, color_t(47, 47, 47, new_alpha));
