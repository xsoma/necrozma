...
void c_colorpicker::render() {
	...
	if (dialogue.open) {
		...
		g_Render->FilledRect(pos.x, pos.y, size.x, 170, color_t(18, 18, 18, alpha));
		g_Render->Rect(pos.x, pos.y, size.x, 170, color_t(58, 58, 58, alpha));
		...
		g_Render->_drawList->AddCircle(ImVec2(pos.x + 15 + (size.x - 30) * saturation, pos.y + 15 + 105.f * (1.f - brightness)), 5, color_t(255, 255, 255, alpha).u32(), 100);
		g_Render->_drawList->AddCircleFilled(ImVec2(pos.x + 15 + (size.x - 30) * saturation, pos.y + 15 + 105.f * (1.f - brightness)), 2, color_t(255, 255, 255, alpha).u32(), 100);
	}
}