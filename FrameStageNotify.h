...
{
	static auto pThis = *reinterpret_cast<DWORD**> (csgo->Utils.FindPatternIDA(GetModuleHandleA(g_Modules[fnv::hash(hs::client_dll::s().c_str())].c_str()),
		hs::pthis::s().c_str() + 1));

	static auto find_hud_element = reinterpret_cast<DWORD(__thiscall*)(void*, const char*)>(csgo->Utils.FindPatternIDA(GetModuleHandleA(g_Modules[fnv::hash(hs::client_dll::s().c_str())].c_str()),
		hs::find_hud_element::s().c_str()));
...
void __stdcall Hooked_FrameStageNotify(ClientFrameStage_t curStage)
{
	...
	if (interfaces.engine->IsConnected() && interfaces.engine->IsInGame())
	{
		...
		if (curStage == FRAME_NET_UPDATE_POSTDATAUPDATE_START && csgo->local->isAlive())
		{
			//features->skinchanger->skinchager();
			features->Visuals->ModelChanger();
			c_skins::skinchager();
			c_skins::glovechanger();
		}
		if (curStage == FRAME_RENDER_START)
		{
			if (csgo->local->isAlive()) {
				...
				auto tf = csgo->local->GetPlayerAnimState();
				if (csgo->local && csgo->local->isAlive() && tf)
					csgo->local->SetAbsAngles(Vector(0, tf->m_flGoalFeetYaw, 0));
				...
				if (csgo->animstate && animstate)
					csgo->desync_angle = clamp(fabsf(Math::NormalizeYaw(csgo->animstate->m_flGoalFeetYaw - animstate->m_flGoalFeetYaw)), 0.f, 120.f);

				//if (csgo->fake_duck)
					//csgo->local->GetDuckAmount() = 0.f;
			}
